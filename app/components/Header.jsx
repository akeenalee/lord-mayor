  ...
  return (
    <Link
      ...
      className={`rounded-full px-4 py-2 text-sm font-medium transition ${
        active ? 'bg-slate-900 text-white shadow-soft' : 'text-slate-700 hover:bg-slate-100 hover:text-slate-950'
      }`}
    >
      {label}
    </Link>
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="flex items-center justify-between py-4">
          <Link href="/" className="flex items-center gap-3" onClick={() => setOpen(false)}>
            <div className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-2xl border border-amber-100 bg-white shadow-soft">
              <Image src="/logo.png" alt="Lord Mayor logo" width={40} height={40} priority />
            </div>
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.35em] text-amber-600">Lord Mayor</p>
              <p className="text-base font-semibold text-slate-950">of Africa Nig Ltd</p>
            </div>
          </Link>
        ...
        <div className={`overflow-hidden transition-all duration-300 md:hidden ${open ? 'max-h-96 pb-5 opacity-100' : 'max-h-0 opacity-0'}`}>
